...
3. **Reroute** — swap a whole module for a brand override (the welcome page). The original source stays byte-for-byte unchanged; `apply.mjs` generates `apps/app/src/brandkit-generated/<name>.tsx` + `apps/app/vite.brandkit.config.mts`. Run dev/build with `vite --config vite.brandkit.config.mts` from `apps/app`.
