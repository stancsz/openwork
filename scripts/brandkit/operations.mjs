...
export function enabledFeatures(config) {
  ...
  return {
    ...
    onboardingProviderOverride:
      f.onboardingProviderOverride && config.onboarding.providerSetup === "settings",
...
export function buildOperations(config) {
  ...
  const ops = [
    ...
    // Replace provider-choice onboarding with brand-owned instructions that
    // point the user to Settings → AI Providers for their API key.
    {
      id: "reroute:provider-selection-override",
      feature: "onboardingProviderOverride",
      type: "writeFile",
      target: "apps/app/src/brandkit-generated/provider-selection-step.tsx",
      content: renderProviderSelectionOverride(config.brand.displayName),
    },
    ...
    // keeps working with welcomeOverride off — the reroute map is just empty then.
    {
      ...
      content: renderBrandViteConfig(
        enabledFeatures(config).welcomeOverride,
        enabledFeatures(config).onboardingProviderOverride,
      ),
    },
...
/** The brand-owned provider setup step for distributors that configure keys in Settings. */
function renderProviderSelectionOverride(brandName) {
  const brand = JSON.stringify(brandName);

  return `/** @jsxImportSource react */
// AUTO-GENERATED by scripts/brandkit — edit the brand onboarding config and
// re-run \`node scripts/brandkit/apply.mjs\`. The stock provider-selection
...
import { Button } from "@/components/ui/button";
import { ArrowRightIcon, KeyRoundIcon, SettingsIcon } from "lucide-react";
...
export function ProviderSelectionStep({ onSkip }: ProviderSelectionStepProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background">
      ...
      <div className="relative z-10 w-full max-w-md px-6">
        <PageHeader className="mb-8 text-center">
          <PageTitle>Set up {BRAND_NAME}</PageTitle>
          <PageDescription>
            Go to Settings → AI Providers to enter your API key, then return here to run your first task.
          </PageDescription>
        </PageHeader>
        ...
        <div className="space-y-4">
          <div
            ...
            data-testid="clinicwork-onboarding-settings-hint"
          >
            <SettingsIcon className="mt-0.5 size-5 shrink-0 text-muted-foreground" />
            <div>
              <div className="text-sm font-medium text-foreground">Enter your API key in Settings</div>
              <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
                Open the settings gear, choose AI Providers, and add the key for your clinic gateway.
              </div>
            </div>
          </div>
 ...
 */
function renderBrandViteConfig(withWelcomeOverride, withProviderSelectionOverride) {
  const overrideEntries = [
    withWelcomeOverride
      ...
      : null,
    withProviderSelectionOverride
      ? `[resolve(APP_ROOT, "src/react-app/domains/onboarding/provider-selection-step.tsx")]:
      resolve(APP_ROOT, "src/brandkit-generated/provider-selection-step.tsx"),`
      : null,
  ].filter(Boolean);
  const overrides = overrideEntries.length
    ? `{\n    ${overrideEntries.join("\n    ")}\n  }`
    : `/* no brand-owned modules rerouted */ {}`;
